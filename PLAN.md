...
3. 缩略图生成服务
   - 生成 256x256 和 512x512 的 WebP 缩略图
   - 异步队列处理，不阻塞UI
4. 网格浏览界面
...
### 验证标准
- [ ] 拖入 10 张图片，全部出现在网格中，缩略图正常显示
- [ ] 数据库 `media` 表有 10 条记录，字段完整
- [ ] `library/` 和 `thumbnails/` 目录结构正确
- [ ] 导入 1000 张图片，滚动流畅无卡顿 (虚拟滚动生效)
- [ ] 支持按 `created_at` / `modified_at` / `imported_at` 排序
...
### 验证标准
- [ ] 导入一张动漫图片，自动出现 `1girl`, `solo`, `long_hair` 等标签
- [ ] 删除一个AI标签后，该图片不再显示此标签
- [ ] 搜索框输入 `tag:cat` 只显示带 cat 标签的图片
- [ ] 选择 5 张图片，批量添加 `favorite` 标签，全部生效
- [ ] 删除标签 `test`，所有关联的 media_tags 记录被级联删除
